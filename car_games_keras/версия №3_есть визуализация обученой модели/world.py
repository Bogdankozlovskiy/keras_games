...
def neuron(car,score,neu):
    '''car-класс содержащий параметры машины, позицию и направление
    Score-входные данные для нейронной сети
    neu-сама нейронная сеть
    Return: новый объект car содержащий новое положение машинки'''
    otvet = np.argmax(neu.predict(score))
    if otvet == 0:
...
def ray(original,face,mapi,car,num_layers=5,sizeshow=(800,600)):
    """
